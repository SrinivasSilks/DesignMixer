package com.vasu.loom2.design4.pallu;
...
public class PalluConversion {
    ...
    public static BufferedImage get(BufferedImage right, BufferedImage left, BufferedImage rani, BufferedImage jari) throws IOException {
        ...
        saveBMP(brocade, format("z-vasu/out/2/design4/pallu-%s-%s.bmp", brocade.getWidth(), brocade.getHeight()));
        return brocade;
    }

    public static void main(final String[] args) throws IOException {
        BufferedImage rightd = ImageIO.read(new File("z-vasu/in/2/design4/border/border.bmp"));
        BufferedImage leftd = VerticalFlipGenerator.get(ImageIO.read(new File("z-vasu/in/2/design4/border/border.bmp")));

        BufferedImage rightf = HorizontalRepeatGenerator.get(60, rightd);
        BufferedImage right = CutLayoutGenerator.get(CutLayoutGenerator.get(rightf, 180, 1), 1920, 0);

        BufferedImage leftf = HorizontalRepeatGenerator.get(60, leftd);
        BufferedImage left = CutLayoutGenerator.get(CutLayoutGenerator.get(leftf, 180, 1), 1920, 0);

        BufferedImage jari = ImageIO.read(new File("z-vasu/in/2/design4/pallu/pallu_jari.bmp"));
