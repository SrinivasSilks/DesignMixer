...
public class GoldJariBrocade {
    ...
    public static BufferedImage get(BufferedImage left, BufferedImage right, BufferedImage rani, BufferedImage gold, BufferedImage nimbu) throws IOException {
        List<BufferedImage> brocades = new LinkedList<>();
        brocades.add(rani(left, right, rani));
        brocades.add(jari(getBorder(left), getBorder(right), nimbu));
        brocades.add(nimbu(getBorder(left), getBorder(right), gold));
    ...
    private static BufferedImage empty(int width) {
        return EmptyGenerator.get(width, 720);
    }
