...
public class KadiyaluPlainConversion {
    ...
    public static BufferedImage get(
        ...
        saveBMP(brocade, format("d/16/out/design2/design-kadiyalu-plain-%s-%s.bmp", brocade.getWidth(), brocade.getHeight()));
        return brocade;
    }

    public static void main(final String[] args) throws IOException {
        BufferedImage raniJari = borderR(ImageIO.read(new File("d/16/in/design2/border/right.bmp")));
        BufferedImage raniSilk = PlainGenerator.get(raniJari.getWidth(), raniJari.getHeight());

        BufferedImage teegaJari = borderL(ImageIO.read(new File("d/16/in/design2/border/teega.bmp")));
        BufferedImage teegaSilk = PlainGenerator.get(teegaJari.getWidth(), teegaJari.getHeight());
        BufferedImage banarasJari = borderL(ImageIO.read(new File("d/16/in/design2/border/repeat.bmp")));
